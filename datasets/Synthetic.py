...
class SyntheticDataset(Dataset):
    ...
    def __init__(self, input_threads=8, load_test=False):
        ...
        self.path = "/cluster/scratch/cakcay/synthetic_real_with_noise"

        _train_list = f"{self.path}/split_train.txt"
        _val_list = f"{self.path}/split_val.txt"
        _test_list = f"{self.path}/split_test.txt"

        # Path of the training files
        self.train_path = join(self.path, 'training_points')
        self.test_path = join(self.path, 'test_points')

        self.split_train = np.loadtxt(_train_list, dtype=np.str)
        self.split_val = np.loadtxt(_val_list, dtype=np.str)
        self.split_test = np.loadtxt(_test_list, dtype=np.str)
        ...
        self.train_files = []
        for r, d, f in os.walk(self.train_path):
            for x in f:
                if x.endswith(".ply"):
                    self.train_files.append(join(r, x))

        self.test_files = []
        for r, d, f in os.walk(self.test_path):
            for x in f:
                if x.endswith(".ply"):
                    self.test_files.append(join(r, x))

        self.train_files = np.sort(self.train_files)
        self.test_files = np.sort(self.test_files)

        # Proportion of validation scenes
        self.validation_clouds = np.loadtxt(_val_list, dtype=np.str)
        self.train_clouds = np.loadtxt(_train_list, dtype=np.str)
        self.test_clouds = np.loadtxt(_test_list, dtype=np.str)
    ...
    def prepare_pointcloud_ply(self):
        ...
        paths = [join(self.path, 'train'), join(self.path, 'test')]
        ...
        for path, new_path, mesh_path in zip(paths, new_paths, mesh_paths):
            ...
            if path.endswith("train"):
                scenes = np.concatenate((self.split_train, self.split_val))
            else:
                scenes = self.split_test
            ...
            for i, scene in enumerate(scenes):
                ...
                # Read mesh
                # !
                vertices, labels = read_synthetic_ply(f"{path}/{scene}.ply") 

                # ! all white
                vertices_colors = np.ones((vertices.shape[0],3)).astype(np.float32) * 255.

                # ! background or human
                vertices_labels = np.zeros(vertices.shape[0], dtype=np.int32)
                vertices_labels[np.logical_or(labels>99, labels==31)] = 1
                ...
                vertices = vertices.dot(align_mat.T)

                _stem = scene.split("/")[:-1]
                _temp_root = mesh_path
                for s in _stem:
                    _temp_root += f"/{s}"
                if not exists(_temp_root):
                    makedirs(_temp_root)
                ...
                # ! We do not have faces, not rasterizing
                
                # Subsample points
                # ! change to 2 cm for compatibility?
                sub_points, sub_colors, sub_labels = grid_subsampling(vertices.astype(np.float32),
                                                                      features=vertices_colors,
    ...
    def load_subsampled_clouds(self, subsampling_parameter):
        ...
        for i, file_path in enumerate(files):
            ...
            cloud_name = file_path.split('_points/')[-1][:-4]
            cloud_folder = file_path.split('_points/')[0]
           
            if 'train' in cloud_folder:
                if cloud_name + ".ply" in self.validation_clouds:
                    self.all_splits += [1]
                    cloud_split = 'validation'
                elif cloud_name + ".ply" in self.train_clouds:
                    self.all_splits += [0]
            ...
            if isfile(KDTree_file):
                ...
                # Points and features have to be float32, and labels (if they are encoding a class attribute) have to be int32.
                # Read ply file
                data = read_ply(file_path)
                points = np.vstack((data['x'], data['y'], data['z'])).T
                colors = np.vstack((data['red'], data['green'], data['blue'])).T
                colors = np.ones((colors.shape[0],3)).astype(np.float32) * 255.
                labels = data["class"]
        ...
        print("num_validation: ", self.num_validation)
        ...
        for i, file_path in enumerate(files):
            ...
            if (not self.load_test) and 'train' in cloud_folder and cloud_name + ".ply" in self.validation_clouds:
                ...
                if isfile(proj_file):
                    ...
                    # Get original mesh
                    mesh_path = file_path.split('_points')
                    mesh_path[0] = mesh_path[0] + '_meshes'
                    mesh_path = '/'.join(mesh_path)
                    vertex_data = read_ply(mesh_path[:-4] + '_mesh.ply', triangular_mesh=False)
    ...
    def load_evaluation_points(self, file_path):
        ...
        # /cluster/scratch/cakcay/synthetic_real_with_noise/training   /recording_20210907_S03_S04_01/main/....ply
        mesh_path = file_path.split('_points')
        mesh_path[0] = mesh_path[0] + '_meshes'
        mesh_path = '/'.join(mesh_path)
        vertex_data = read_ply(mesh_path[:-4] + '_mesh.ply', triangular_mesh=False)
